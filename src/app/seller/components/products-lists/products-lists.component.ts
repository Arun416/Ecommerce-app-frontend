...
export class ProductsListsComponent implements OnInit {
 ...
 /*  _productLists:any = [
  ...
  constructor(private productServ:ProductService,private token: TokenStorageService) { }
  ...
  getMyProducts(){
    this.productServ.getProductsBySeller(this.getUser().data.seller_Id).subscribe({
      next:(response:any)=>{
        console.log(response);
        this._productLists = response.item[0]
      }
    })
  }
